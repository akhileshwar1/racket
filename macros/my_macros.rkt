                        ...
                        #,@(for/list ([x (syntax->list #'(fields ...))]
                                   ...
                                   (with-syntax ([acc-id (format-id #'id "~a-~a" #'id x)]
                                                ...
                                                #`(define (acc-id v)
                                                    ...
                                                    (vector-ref v ix))))))]))
;;(expand/step-text (our-struct second (id sid)))
#;(let ([x 1] [y 2])
     (second x y)
     )
...
;; (for/list ([var lst]) body0 body ...) -> (define a function with params as (list)  that accumulates in a list and call it as well.
...
(let ([i 0])
  (while (< i 5) (displayln i) (set! i (+ i 1)))) ; kind of seems useless in the sense that the outer let should have been redundant if we used the inner named let purposefully.
